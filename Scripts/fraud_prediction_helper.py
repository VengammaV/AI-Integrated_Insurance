...
def preprocess_input(input_dict):
    expected_columns = [
    "claim_amount", "income", "suspicious_flag", "claim_processing_days", "claim_to_income_ratio", 'anomaly_suspicious_flag',
    ...
    # Add anomoly_suspicious_flag feature to the dataset:
    iso_out = iso.fit_predict(df[cols_to_scale])

    # Combine into suspicious flag
    votes = ((pd.DataFrame({
        'iso': iso_out
    }) == -1).sum(axis=1))

    df['anomaly_suspicious_flag'] = (votes >= 2).astype(int)
